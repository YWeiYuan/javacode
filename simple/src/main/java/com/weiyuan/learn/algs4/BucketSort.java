 ...
 * Copyright 2012-2022
 */
package com.weiyuan.learn.algs4;
 ...
 * 每个桶子再个别排序（有可能再使用别的排序算法或是以递归方式继续使用桶排序进行排序）。桶排序是鸽巢排序的一种归纳结果。当要被排序的数组内的数值是均匀分配的时候，桶排序使用线性时间（Θ（n））。但桶排序并不是 比较排序，他不受到 O(n log n) 下限的影响。<br>
 * <br>执行流程如下<br>
 * 1 设置一个定量的数组当作空桶子。<bR>
...
public class BucketSort {
    ...
    private static void sort(List<Integer> sequenceList, int divisionValue) {
        ...
        List<Integer>[] bucketArray = new ArrayList[10];

        // 2 迭代待排序序列，并放到对应的桶中
        for (int value : sequenceList) {
            //通过该运算获取到我们需要排序的那一位数字，并放入到对应桶中
            int index = value / divisionValue % 10;
        ...
        if (divisionValue > 1) {
            for (List<Integer> bucketDataList : bucketArray) {
                if (bucketDataList != null && bucketDataList.size() > 0) {
                    sort(bucketDataList, divisionValue / 10);
                }
            }
        }
